...
Das Verfahren ist im Algorithmus \textsc{RandomizedObtusePath} zusammengefasst. $z$ bezeichnet ein Array, das die Ortsvektoren aller Punkte enthält, sodass $\vec z_i$ der Ortsvektor des $i$-ten Punkts, wie oben definiert, ist. Damit die Namen im Pseudocode mit denen im echten Code übereinstimmen, ist er auf Englisch. \emph{path} enthält den aktuellen Pfad, \emph{extendingBack} gibt an, ob der Pfad gerade vorne oder hinten erweitert wird, und \emph{noAddedNode} ist der Zähler für die Anzahl konsekutiver Neuverbindungen (oder, äquivalent, der Anzahl an Ausführungen der while-Schleife, in denen kein Knoten zu \emph{path} hinzugefügt wurde). In der while-Schleife wird zunächst geprüft, ob das Limit von \emph{noAddedNode} überschritten wurde, wobei sich $n/2$ experimentell als gutes Limit herausgestellt hat. Ein zu großes Limit sorgt für eine größere Laufzeit, wenn ein ohnehin aussichtsloser Pfad noch lange versucht wird, zu erweitern, mit einem zu kleinen Limit wird die Suche dagegen sehr schnell abgebrochen, obwohl eine Erweiterung möglicherweise doch noch möglich gewesen wäre. Denn manchmal ist eine längere Folge an Neuverbindungen nötig, um den Pfad wieder erweiterbar zu machen. $u$ bezeichnet anschließend den Endknoten des Pfads, $v$ einen zufällig ausgewählten, der nach bzw. vor $u$ angefügt werden könnte. Gibt es einen solchen Knoten $v$, wird der Pfad erweitert und \emph{noAddedNode} zurückgesetzt. Andernfalls wird \emph{noAddedNode} erhöht und das aktuell behandelte Ende des Pfads als Sackgasse (,,dead end'') markiert. Wenn nicht beide Seiten als Sackgasse markiert sind, wird versucht, den Pfad am anderen Ende zu erweitern, indem \emph{extendingBack} invertiert wird (vorletzte Zeile, nach dem \textbf{else}). Andernfalls 
