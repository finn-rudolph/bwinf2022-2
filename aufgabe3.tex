...
\begin{algorithm}
    ...
    \While{$Q \ne \{\}$}
    {
        $(n, i) \gets $ \textsc{Dequeue}$(Q)$ \;
        $s \gets$ \textsc{IthPermutation}$(n, i)$ \;
        \If{$\mu(p) = 0$}
        {
            \Return{} \textsc{ReconstructOperations}$(pre, n, i)$ \;
        }
        ...
        \For{$i \gets 0$ \KwTo $|s| - 1$}
        {
            \If{$\gamma_i s \notin pre\mathrm{[|s| - 1]}$}
            {
                $pre[|s| - 1][\mu(\gamma_i s)] \gets \mu(s)$ \;
                \textsc{Enqueue}$(Q, (n - 1, \mu(\gamma_i s)))$ \;
            }
        }
    }
...
\noindent Um die durchgeführten $\gamma_i$-Operationen aus \emph{pre} zu rekonstruieren, wird der Suchbaum von unten nach oben durchlaufen. Auf dem direkten Vorgänger wird jede mögliche $\gamma_i$-Operation ausprobiert, bis die Richtige gefunden ist.

\begin{algorithm}
    $t \gets$ empty Array

    \While{$n < \mathrm{size}$ $\mathrm{of}$ $pre$}{
        $s \gets$ \textsc{IthPermutation}$(n + 1, pre[n - 1][i])$ \;
    ...
    \caption{\textsc{ReconstructOperations}$(pre, n, i)$}
\end{algorithm}
