...
\begin{algorithm}
    ...
    \While{$Q \ne \emptyset$}
    {
        ...
        \For{$j \gets 0$ \KwTo $\mathrm{length} - 1$}
        {
            \If{$\mu(\gamma_j s) \notin \mathrm{pre[length - 2]}$}
            {
...
\begin{algorithm}
    ...
    \While{$\mathrm{length} < n$}{
        $s \gets$ \textsc{IthPermutation}$(\text{length} + 1, \text{pre}[\text{length} - 1][\text{index}])$ \;

        \For{$j \gets 0$ \KwTo $\mathrm{length} + 1$}
        {
            \If{$\mu(\gamma_j s) = \mathrm{index}$}
            {
                append $j$ to the front of operations \;
                \textbf{break} \;
            }
        }

        index $\gets$ pre[length - 1][index] \;
...
\begin{algorithm}
    ...
    \While{$Q \ne \emptyset$}
    {
        ...
        \If{$\mathrm{lbound} \ge \mathrm{ubound}$}
        {
            \textbf{break} \;
        }
        ...
        \If{$\mathrm{index} = 0$}
        {
            \If{$n - \mathrm{length} < \mathrm{ubound}$}
            {
        ...
        \For{$j \gets 0$ \KwTo $\mathrm{length} - 1$}
        {
            \If{$\mu(\gamma_j s) \notin \mathrm{pre[length - 2]} \land n - \mathrm{length} + \text{\textsc{LowerBound}}(\gamma_j s) + 1 < \mathrm{ubound}$}
            {
    ...
    \Return{\textsc{ReconstructOperations}$(\mathrm{pre}, n - \mathrm{ubound}, 0)$} \;
...
\begin{algorithm}
    ...
    \While{$Q \ne \emptyset$}
    {
        ...
        \If{$\mathrm{found} \land |\mathrm{operations}| + 1 < \mathrm{ubound}$}
        {
